...
def clean_address(raw, county=None):
    ...
    if m:
        s = m.group(1)
    s = re.sub(r'\([^()]*\)?', '', s)
    # 據點名用連字號接在門牌前面，TGOS 比不到。兩種形態都實測過：
    #   「臺北市大安區吉利分館--臺北市北投區立農街1段366號」前後縣市**互相矛盾**，
    #     前段是所屬單位、後段才是真正的門牌所在，所以連字號後接得出縣市時一律取後段。
    #   「臺北市北投區北投國中教學點-北投區溫泉里溫泉路62號」後段只有行政區沒有縣市，
    #     這時保留原本的縣市，只砍掉中間的據點名（重複的行政區交給下面第 3 條收掉）。
    m = re.search(rf'[-－]{{1,2}}((?:{COUNTY_RE}).*)$', s)
    if m:
        s = m.group(1)
    else:
        m = re.match(rf'^((?:{COUNTY_RE})).*?[-－]{{1,2}}([^0-9]{{1,4}}?[區鄉鎮市].*)$', s)
        if m:
