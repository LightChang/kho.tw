...
### 接名錄類來源（`entity: 'venue'`）前先讀這段

名錄不是「多接多好」。`transform/resolve-relations.mjs` 一旦用名稱或門牌對到名錄，
場館的 id 就從 `hash8(門牌)` 換成 `hash8(名錄來源|記錄)`——**如果那筆名錄自己沒有座標、
它的門牌又不在 `geocode/out/geocoded.csv` 裡，原本靠門牌直接命中快取而有的座標就沒了。**

**量淨效果要用課程層，不可以用場館 id。** 對到名錄之後場館 id 本來就會變
（`hash8(門牌)` → `hash8(名錄來源|記錄)`），拿「舊 id 還在不在、有沒有 lat」當判準
會得到一堆假警報。2026-09-13 接學校名錄時我先用了這個錯判準，報出「36 個場館、
152 門課失去座標」，加了守門後還剩 7 個；逐一追查才發現**一門課都沒掉**——
那些門牌全被學校名錄接手，接手者的 lat 與原值一模一樣，只是換了 id 和一個更好的名字
（「桃園市桃園區三民路3段22號」變成「市立成功國小」）。

正確作法：把該來源的 `data/observation/<id>.ndjson` 暫時移走跑一次 `resolve-relations`，
存下 `relations.ndjson` 與 `venues.ndjson`，移回再跑一次，然後**以 `relations` 的
`fromId`（課程 id）為鍵**比對每門課的場館有沒有座標。學校名錄的實測結果是：
座標新得 3 門、失去 0 門，行政區 +189 門，`derived-name` 從 4,854 降到 4,260。

順帶一提這個結果的意思：名錄本身沒有座標時，接它只換得到**名字與行政區**，換不到座標。
