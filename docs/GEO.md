...
間接訊號可以看流量來源（GA 的參照連結會出現 chatgpt.com、perplexity.ai 這類網域）：

```bash
node scripts/google.mjs ga [天數]
```

**注意**：目前 `ga` 這個指令只回報使用者、工作階段、瀏覽次數，**沒有**參照來源維度。
要看參照來源請用 GA 網頁介面，或之後把維度加進 `scripts/google.mjs` 的 `ga()`。
這裡照實寫出工具的限制，不假裝指令做得到。
