...
# ── 4. 產生網站並驗證 ──────────────────────────────────────
# 直接用 $NODE 跑 Astro 的 CLI 檔，不走 pnpm exec：launchd 的 PATH 沒有 nvm，找不到套件管理器，
# 而 node 本身已經在上面驗過是可執行的。node_modules 不在（沒跑過 pnpm install）就明講。
...
# 否則排程跑的驗證會比手動 pnpm run validate 弱，而且不會有人發現。
